...
export class SAT {
  public static collidePolygons(polyA: Vector2[], polyB: Vector2[]): boolean {
    ...
    for (const axis of axes) {
      ...
      if (!this.overlap(projA, projB)) return false;
    }
    return true;
  }

  private static getAxes(poly: Vector2[]): Vector2[] {
    const axes: Vector2[] = [];
  ...
  private static project(poly: Vector2[], axis: Vector2): [number, number] {
