...
export class World {
  ...
  public addComponent(entityId: EntityID, component: Component): void {
    ...
    if (map) map.set(component.type, component);
  }

  public getComponent<T extends Component>(entityId: EntityID, type: string): T | undefined {
    return this.components.get(entityId)?.get(type) as T;
  }
  ...
  public getEntitiesWith(componentTypes: string[]): EntityID[] {
    ...
    for (const [id, compMap] of this.components.entries()) {
      let match = true;
      for (const type of componentTypes) {
        if (!compMap.has(type)) { match = false; break; }
      }
