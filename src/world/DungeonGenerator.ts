...
export class DungeonGenerator {
  public static generate(width: number, height: number): Tilemap {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        if (x === 0 || x === width - 1 || y === 0 || y === height - 1) {
          ...
          map.setTile(x, y, Math.random() > 0.8 ? 1 : 0);
        }
      }
    }
    return map;
